...
def test_log_activity_writes_event_and_fields(caplog) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="bob.activity"):
        log_activity("route.heuristic", specialists=["crm_intelligence_specialist"], query="east")
    assert "route.heuristic" in caplog.text
...
def test_configure_logging_writes_file(tmp_path: Path, monkeypatch) -> None:
    path = tmp_path / "bob.log"
    monkeypatch.setenv("BOB_LOG_FILE", str(path))
    monkeypatch.setenv("BOB_LOG_LEVEL", "INFO")
